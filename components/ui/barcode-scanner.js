...
const BarcodeScanner = function (props) {
    ...
    useEffect(() => {
        ...
        Quagga.onProcessed(function(result) {
            ...
            const scale = video.clientWidth / parseInt(drawingCanvas.getAttribute("width"));
            ...
            if (result) {
                ...
                if (result.codeResult && result.codeResult.code) {
                    const line = result.line.map((c) => ({ x: scale * c.x, y: scale * c.y }))
                    Quagga.ImageDebug.drawPath(line, {x: 'x', y: 'y'}, drawingCtx, {color: 'red', lineWidth: 3});
                }
            }
        });
